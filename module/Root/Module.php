...
class Module {
	...
	public function authPreDispatch(MvcEvent $event) {
		...
		if (! in_array ( $event->getRouteMatch ()->getMatchedRouteName (), $routes_non_secured )) {
			...
			if ($identity) {
				...
				$event->getViewModel ()->setVariable ( 'username', $event->getApplication ()->getServiceManager ()->get ( 'Root\Model\MyAuthStorage' )->read () );
			}
	...
	public function init(ModuleManager $mm) {
		$mm->getEventManager ()->getSharedManager ()->attach ( __NAMESPACE__, 'dispatch', function ($e) {
			$e->getTarget ()->layout ( 'Root/layout' );
		} );
	}
	...
	public function getServiceConfig() {
		return array (
				"factories" => array (
						"Root\Model\MyAuthStorage" => function ($serviceManager) {
							return new \Root\Model\MyAuthStorage ( "myauthstorage" );
						},
